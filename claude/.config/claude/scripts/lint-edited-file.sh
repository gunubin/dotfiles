...
if [[ "$tool_name" =~ ^(Edit|MultiEdit|Write)$ ]]; then
    ...
    if [ -n "$file_path" ] && [ -f "$file_path" ]; then
        ...
        if [ -n "$project_root" ]; then
            ...
            # package.jsonにlint:fixスクリプトがあるか確認
            has_lint_fix=$(jq -r '.scripts["lint:fix"] // ""' package.json)
            
            if [ -n "$has_lint_fix" ]; then
                ...
                if [[ "$extension" =~ ^(ts|tsx|js|jsx|mjs|cjs)$ ]]; then
                    ...
                    if [ -n "$relative_path" ] && [ -f "$project_root/$relative_path" ]; then
                        # lint:fixを実行（ファイルパスをクォート）
                        log_message "[$(date)] Running lint:fix on: $relative_path"
                        if [ "$ENABLE_LOGGING" = "true" ]; then
                            npm run lint:fix -- "$relative_path" 2>&1 >> ~/.claude/lint-hook.log
                        else
                            npm run lint:fix -- "$relative_path" 2>&1 > /dev/null
                        fi
                        
                        # 結果をログ出力
                        log_message "[$(date)] Linted: $file_path (project: $project_root)"
                    else
                        log_message "[$(date)] Invalid relative path: $relative_path (file: $file_path)"
                    fi
                fi
            else
                log_message "[$(date)] No lint:fix script found in $project_root"
            fi
        else
