...
def get_percentage_color(percentage):
    ...
    if percentage >= 90:
        return '\033[38;2;243;139;168m'  # Catppuccin Red #f38ba8
    elif percentage >= 70:
        return '\033[38;2;250;179;135m'  # Catppuccin Peach #fab387
    return '\033[38;2;166;227;161m'      # Catppuccin Green #a6e3a1
...
def create_sparkline(values, width=20):
    ...
    for i in range(data_width):
        ...
        if idx < len(values):
            ...
            # Color based on value (Catppuccin gradient)
            if normalized > 0.7:
                color = '\033[38;2;250;179;135m'  # Peach #fab387
            elif normalized > 0.4:
                color = '\033[38;2;249;226;175m'  # Yellow #f9e2af
            else:
                color = '\033[38;2;148;226;213m'  # Teal #94e2d5
...
def format_output_full(ctx, terminal_width=None):
    ...
    # Line 5+: Recent user prompts (1 line each, newest last)
    prompts = ctx.get('recent_prompts', [])
    if prompts:
        tw = terminal_width or get_terminal_width()
        clean = lambda t: re.sub(r'<[^>]*>', '', t).replace('\n', ' ').strip()
        prefix = "> "
        max_len = tw - len(prefix)
        # Show up to 2 prompts: if last is short, also show the previous
        last = clean(prompts[-1])
        show = []
        if len(prompts) >= 2 and len(last) < max_len * 0.6:
            show.append(clean(prompts[-2]))
        show.append(last)
        for p in show:
            if len(p) > max_len:
                p = p[:max_len - 1] + "\u2026"
            lines.append(f"{Colors.BRIGHT_MAGENTA}{Colors.BOLD}>{Colors.RESET} {Colors.DIM}{p}{Colors.RESET}")
