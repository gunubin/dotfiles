...
name: cc-ask-reply
description: |
  cc-askのQ&Aセッションで応答を相手paneに転送する。
...
1. **引数パース**
   ...
   - 省略時は最新のセッションファイルを特定（Bash）:
     ```bash
     ls -t ~/.claude/dialogues/cc-ask-*.md | head -1
     ```
     ファイル名からセッション ID を抽出する（`cc-ask-` から `.md` を除いた部分）
...
3. **YAML フロントマターから状態取得**
   - `status`: セッション状態
   - `current_round`: cc-ask-reply の実行回数（`## Round N` のヘッダー番号とは独立した内部カウンター）
   - `max_rounds`: 最大ラウンド数
...
6. **継続処理**（終了条件に該当しない場合）
   ...
   - **Pane B（回答者）が実行した場合** → Pane A（質問者）に送信（Bash。**必ず1つの Bash コマンドとして `&&` で繋いで実行すること**）:
     ```bash
     tmux send-keys -t "{pane_a}" -l -- '~/.claude/dialogues/{session-id}.md のQ&Aセッション、回答が届きました。ファイルを Read で読み、追加質問があれば追記して /cc-ask-reply {session-id} を実行、不明点が解消されたら [CONCLUDED] を含めて追記し /cc-ask-reply {session-id} を実行してください。' && sleep 0.3 && tmux send-keys -t "{pane_a}" Enter
     ```

   - **Pane A（質問者）が実行した場合** → Pane B（回答者）に送信（Bash。**必ず1つの Bash コマンドとして `&&` で繋いで実行すること**）:
     ```bash
     tmux send-keys -t "{pane_b}" -l -- '~/.claude/dialogues/{session-id}.md のQ&Aセッション、追加質問が届きました。ファイルを Read で読み、回答を追記して /cc-ask-reply {session-id} を実行してください。' && sleep 0.3 && tmux send-keys -t "{pane_b}" Enter
     ```
...
- cc-chat-reply とは異なり、ロール（質問者/回答者）に応じてメッセージ内容が変わる
